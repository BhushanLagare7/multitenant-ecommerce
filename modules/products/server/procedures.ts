  ...
  getOne: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (session.user) {
        ...
        isPurchased = orderData.docs.length > 0;
      }
  ...
  getMany: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.tenantSlug) {
        ...
        // Make sure not to load products set to "isPrivate: true" (using reverse not_equals logic)
      ...
      if (input.category) {
        ...
        const formattedCategories = categoriesData.docs.map((doc) => ({
          ...doc,
      ...
      const products = await ctx.db.find({
        collection: "products",
        depth: 2, // Populate "category", "image", "tenant" & "tenant.image" fields
        where,
        sort,
        page: input.cursor,
        limit: input.limit,
